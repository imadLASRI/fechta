...
class PlacesController extends VoyagerBaseController
{
    ...
    public function index(Request $request)
    {
        ...
        // filering places by Manager( showing only places added by the logedid manager)
        // if super admin is connected dont filter
        if(Auth::user()->role_id != 1){
            $temp = collect();

            foreach($dataTypeContent as $place){
                if($place->manager_id == Auth::user()->id){
                    $temp->push($place);
                }
            }

            $dataTypeContent = $temp;
        }
    ...
    public function store(Request $request)
    {
        ...
        //  CUSTOM STORE 
        // casting array to string with ; separator
            // formating array to string (voyager way of storing arrays)
        $stringArr = "[";
        $arr = $request['place_types'];
        for($i = 0; $i<count($arr); $i++){
            $stringArr .= '"' .$arr[$i] . '",';
        }
        $stringArr[strlen($stringArr)-1] = "]";

        // dd($request['place_types'], $stringArr);

        $data = Place::create([
            'manager_id' => Auth::user()->id,
            'place_name' => $request['place_name'],
            'place_city' => $request['place_city'],
            'place_adresse' => $request['place_adresse'],
            'place_phone' => $request['place_phone'],
            'place_types' => $stringArr,
            'place_description' => $request['place_description'],
            'place_image' => $request->file('place_image')->store('places', ['disk' => 'public']),
