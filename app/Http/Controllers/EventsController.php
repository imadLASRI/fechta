...
class EventsController extends VoyagerBaseController
{
    ...
    public function index(Request $request)
    {
        ...
        if(Auth::user()->role_id != 1){
            ...
            foreach($dataTypeContent as $event){
                if($event->places->manager_id == Auth::user()->id){
                    $temp->push($event);
                }
            }
    ...
    public function edit(Request $request, $id)
    {
        ...
        // sending manager places to edit add event view
        if(Auth::user()->id != 1){
            $managerPlaces = Place::where('manager_id', Auth::user()->id)->get();
        }
        else{
            $managerPlaces = Place::all();
        }

        return Voyager::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable', 'managerPlaces'));
    }
